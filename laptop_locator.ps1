...
# This is what you use for attachments. If so, call it like [-attachment "c:\path\to\file.name"] in the below invocation (bottom-most line of this script)
#
...
# Let's get your machine's IP by visiting an external site and setting result to a variable.
...
# Let's get some location information by passing that variable to IPInfoDB.
...
# Let's get some reverse DNS on that IP using PowerShell
#
...
# Let's go ahead and send an email with the results.
# Modify the values here. You'll need to store credentials in plaintext, so don't use important ones, capiche?
